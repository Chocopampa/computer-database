...
public class ComputerMapper {
	...
	public List<Computer> mapList(ResultSet listComputerDb) throws SQLException {
		...
		while(listComputerDb.next()) {
			...
			Company company = new Company.Builder(listComputerDb.getLong("company_id"))
					.build();
			
			Computer computer = new Computer.Builder(listComputerDb.getString("name"))
					.withId(listComputerDb.getInt("id"))
					.withIntroduced(timeIntroduced)
					.withDiscontinued(timeDiscontinued)
					.withCompany(company)
					.build();
	...
	public Computer mapUnique(ResultSet computerDb) throws SQLException {
		...
		while(computerDb.next()) {
			...
			Company company = new Company.Builder(computerDb.getLong("company_id"))
					.build();
					
			computer = new Computer.Builder(computerDb.getString("name"))
					.withId(computerDb.getInt("id"))
					.withIntroduced(timeIntroduced)
					.withDiscontinued(timeDiscontinued)
					.withCompany(company)
					.build();
